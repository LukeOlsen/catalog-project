...
engine = create_engine(
    'sqlite:///clothingstorewithusers.db?check_same_thread=False')
...
def renderItemGroups():
    ...
    if login_session:
        if 'username' in login_session:
            ...
            return render_template('groupsgeneral.html',
...
def renderItemGroupList(clothing_group_id):
    item_group = session.query(ClothingGroup).filter_by(
        id=clothing_group_id).one()
    items = session.query(ClothingItem).filter_by(
        item_group_id=clothing_group_id).all()
    if login_session:
        if 'username' in login_session:
            return render_template('itemlist.html',
                                   items=items, item_group=item_group)
        else:
            return render_template('itemlistgeneral.html',
                                   items=items, item_group=item_group)
    else:
        return render_template('itemlistgeneral.html',
                               items=items, item_group=item_group)
...
@app.route('/clothing/<int:clothing_group_id>/newItem',
           methods=['GET', 'POST'])
def addNewItem(clothing_group_id):
    ...
    if request.method == 'POST':
        ...
        newItem = ClothingItem(name=request.form['name'],
                               ...
                               item_group_id=clothing_group_id,
                               user_id=user_id)
        session.add(newItem)
        session.commit()
        return redirect(url_for('renderItemGroupList',
                                clothing_group_id=clothing_group.id))
    else:
...
def deleteItem(item_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        return redirect(url_for('renderItemGroupList',
                                clothing_group_id=deletedItem.item_group_id))
    else:
